 ...
 */
function theme_gitam_page_init(moodle_page $page) {
    $page->requires->jquery();
    $page->requires->js('/theme/gitam/javascript/theme.js');
}
 ...
 * @return string
 */
function theme_gitam_process_css($css, $theme) {
    global $OUTPUT, $CFG;
    // Set the background image for the logo.
    $logo = $theme->setting_file_url('logo', 'logo');
    $css = theme_gitam_pre_css_set_fontwww($css);
    // Set custom CSS.
    $customcss = $theme->settings->customcss;
    $css = theme_gitam_set_customcss($css , $customcss);
    return $css;
}
 ...
 */
function theme_gitam_set_logo($css, $logo) {
    $tag = '[[setting:logo]]';
    $replacement = $logo;
 ...
 * @return bool
 */
function theme_gitam_pluginfile($course, $cm, $context, $filearea, $args, $forcedownload, array $options = array()) {
    static $theme;
 ...
 * @return string
 */
function theme_gitam_serve_css($filename) {
    global $CFG;
    if (!empty($CFG->themedir)) {
        $thestylepath = $CFG->themedir . '/gitam/style/';
    } else {
        $thestylepath = $CFG->dirroot . '/theme/gitam/style/';
    }
    ...
    if ((($ifmodifiedsince) && (strtotime($ifmodifiedsince) == $lastmodified)) || $etagheader == $etagfile) {
        theme_gitam_send_unmodified($lastmodified, $etagfile);
    }
    theme_gitam_send_cached_css($thestylepath, $filename, $lastmodified, $etagfile);
}
 ...
 */
function theme_gitam_send_unmodified($lastmodified, $etag) {
 ...
 * @param type|string $etag
 */
function theme_gitam_send_cached_css($path, $filename, $lastmodified, $etag) {
    global $CFG;
 ...
 */
function theme_gitam_set_customcss($css, $customcss) {
 ...
 */
function theme_gitam_get_html_for_settings(renderer_base $output, moodle_page $page) {
    global $CFG;
    $return = new stdClass;
 ...
 * @return type|string
 */
function theme_gitam_pre_css_set_fontwww($css) {
    global $CFG, $PAGE;
    ...
    $tag = '[[setting:fontwww]]';
    $theme = theme_config::load('gitam');
    $css = str_replace($tag, $themewww.'/gitam/fonts/', $css);
    return $css;
}
 ...
 * @return type|string
 */
function theme_gitam_set_fontwww() {
    global $CFG, $PAGE;
    if (empty($CFG->themewww)) {
        $themewww = $CFG->wwwroot."/theme";
    } else {
    ...
    $theme = theme_config::load('gitam');
    $fontwww = '$fontwww: "'.$themewww.'/gitam/fonts/"'.";\n";
    return $fontwww;
}
 ...
 * @return type|string
 */
function theme_gitam_render_slideimg($p, $sliname) {
    global $PAGE, $OUTPUT;
    $nos = theme_gitam_get_setting('numberofslides');
    $i = $p % 3;
    // Get slide image or fallback to default.
    $slideimage = '';
    if (theme_gitam_get_setting($sliname)) {
 ...
 * @return type|string
 */
function theme_gitam_get_setting($setting, $format = true) {
    global $CFG;
